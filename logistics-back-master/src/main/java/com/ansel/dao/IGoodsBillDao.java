...
public interface IGoodsBillDao extends JpaRepository<GoodsBill, Long> {
	...
	@Query(value = "select * from goodsbill where goods_bill_code in (select goods_bill_detail_id from cargoreceiptdetail d inner join cargoreceipt c where d.goods_revert_bill_id = c.goods_revert_bill_code and c.back_bill_state = ?1)", nativeQuery = true)
	public List<GoodsBill> transferState(String type);
